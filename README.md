...
### **strawberry_game.py**
- **함수**
  - `strawberry_game(current_player, friend_list, is_friend)`
    - **설명**: 박자에 맞춰 '딸기'를 외치는 게임 🍓👏
    - 매개변수:
      - `current_player (str)`: 현재 차례인 플레이어 객체
      - `friend_list (list[str])`: 현재 차례의 플레이어를 제외한 참가자 배열
      - `is_friend (bool)`: False - 현재 플레이어는 실제 사용자 
                            True - 현재 플레이어는 컴퓨터 사용자
    - 반환값:
      - `[Player]`: 벌주 한잔을 마셔야하는 플레이어 객체
